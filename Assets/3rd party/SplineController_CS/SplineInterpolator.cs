...
public class SplineInterpolator : NetworkBehaviour
{
	...
	int mCurrentIdx = 1;
	[SyncVar]
	public int payloadTier = 1;

	public GameObject upgradeCrate;
	public GameObject upgrade;

	public static bool upgradeExists;
	...
	[Server]
	public void SpawnUpdgradeIfNotDone () {
		if (!upgradeExists) {
			int rand = Random.Range (0, spawnPoints.Length);
			upgrade = GameObject.Instantiate (upgradeCrate, spawnPoints [rand].position, spawnPoints [rand].rotation);
			upgradeExists = true;
			NetworkServer.Spawn (upgrade);
		}
	}

	[ClientRpc]
	public void RpcExplode () {
		Instantiate (spawnThing, transform.position, new Quaternion (0, 0, 0, 1));
	}
	...
	void Update()
	{
		...
		if (isServer) {
			if (mCurrentTime / controller.Duration < (float)payloadTier / 4f) {
				if (contestants == 1)
					mCurrentTime += Time.deltaTime * .5f;
				else if (contestants > 1)
					mCurrentTime += Time.deltaTime;
			} else {
				SpawnUpdgradeIfNotDone ();
			}

			if (mCurrentTime >= controller.Duration && !spawnedThing) {
				RpcExplode ();
				spawnedThing = true;
			}
		}
	...
	void OnTriggerEnter(Collider coll){
		if (coll.tag == "Player") {
			if (coll.transform.root.GetComponent<UpgradePlayerHook> ().carrying) {
				coll.transform.root.GetComponent<UpgradePlayerHook> ().carrying = false;
				payloadTier++;
				upgradeExists = false;
			}
			contestants += 1;
